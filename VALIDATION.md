...
## Final Polish — Flight-Deck Hierarchy — 2026-08-15

The final flight-deck refinement keeps target guidance, velocity, and active contracts as the strongest telemetry. Idle Skyrail contract timing is now intentionally withheld, and an idle local objective suppresses its dormant progress rail and elapsed-record line while retaining the route name, state, and instruction. The City Atlas control now announces its keyboard shortcut and has an explicit focus treatment; transient system text is exposed through a dedicated polite status region rather than making the entire HUD a live region.

`pnpm check` passed after these changes. The new 1280×720 launch capture confirms the dark city composition, high-contrast launch action, two clearly differentiated operator choices, and reachable Settings entry point remain stable. A browser-level post-launch inspection confirmed the running HUD exposes the target vector, City Atlas, velocity, idle Skyrail contract, compact idle objective, and notification. The browser automation session subsequently reset after the in-scene transition, so final in-world image verification remains part of the full validation phase using the deterministic preview route.

The deterministic `/?demo` route now renders at both 1280×720 and 375×812. The target vector and City Atlas retain clear prominence, the idle contract no longer displays inactive timing, and the mobile layout suppresses the peripheral contract rail. Visual review exposed a minor lower-left stacking seam between the velocity panel and compact objective rail; the mobile speed panel is now intentionally removed and the objective occupies a clean, dedicated lower-edge position. The corrected phone capture confirms the collision is resolved.

The live deterministic session exposes City Atlas with the accessible title `Open City Atlas (M)` and the expected map control in the running flight deck. The browser automation capture upload failed immediately after activating that control and reset the remote viewport; this is logged as an automation limitation, not a reproduced game defect. The atlas itself remains covered by the existing integrated-loop validation, while the fresh deterministic route validates the related live player, waypoint, objective, and control state.

The release-candidate gates were rerun against the final source state: `pnpm check` completed cleanly and Vitest passed **22 tests in 11 files**. The client-console history contains an older lifecycle error from before the managed preview restart, but a timestamp-scoped review found no client errors, exceptions, or failed runtime events after that restart. The only remaining development-server notice is the non-blocking baseline-browser-mapping freshness advisory. The release decision is therefore **approved for the validated static preview runtime**; the known sandbox memory ceiling still excludes production bundling from this local gate.

## Autonomous Upgrade — Flow Circuit — 2026-08-16

The selected autonomous upgrade is a durable free-traversal mastery loop rather than a second time trial or presentation-only counter. Flow Circuit observes the existing web link, release, vector zip, and wall-kick signals; requires distinct high-speed actions; expires cleanly with the current chain window; and persists both best circuit size and completed count locally. The deterministic desktop route visibly renders its active compact rail while preserving clear target-vector, route-contract, and local-objective hierarchy. The current mobile deterministic route renders the city correctly and intentionally suppresses the Flow rail, retaining the dedicated lower-edge objective position.

The exact release source passes `pnpm check` and **25 Vitest assertions across 12 files**, including three focused Flow Circuit checks for distinct-action completion, duplicate rejection/expiry, and browser-storage restoration. Recent browser-console and network logs contain no exception or HTTP-failure evidence; current-day development logs contain only HMR updates, with no errors. The optional full-page WebGL visual-review capture returned black canvases, so its style feedback is excluded as unreliable tooling output; standard desktop and mobile canvas captures remain the validated visual evidence. The release decision is **approved for the static preview runtime**. Production bundling remains outside this sandbox gate because Babylon dependency transformation previously exceeded available memory.
